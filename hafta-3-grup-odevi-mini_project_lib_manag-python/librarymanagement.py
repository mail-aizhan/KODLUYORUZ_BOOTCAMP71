...
class Library:
    ...
    def search_book(self):
        ...
        g2 = str(input("Please write the book's name or author, according to your previous choice: ")).title()
        if g == "n":
            for bid in self.book_objects:
                if self.book_objects[bid][0] == g2:
                    print(f"Found! Bid: {bid} Name: {str(self.book_objects[bid][0])}\nNumber of Copies: {str(self.book_objects[bid][1])} Authors: {str(self.book_objects[bid][2])}")
                    return 0
            print("Not found!\n")
        if g == "a":
            for bid in self.book_objects:
...
class Main:
    ...
    def login(self):
        ...
        while True:
            ...
            if UserDB().validate_user(username,password) == True:
                if UserDB().users_objects[username][1] == "admin":
                    self.show_admin_menu()
                elif UserDB().users_objects[username][1] == "student":
        ...
        while True:
            ...
            if option == 1:
                for item in Library(False).list_book():
                    print(f"Bid: {item} ||| Name: {Library(False).list_book()[item][0]}\nNumber of Copies: {Library(False).list_book()[item][1]} ||| Authors: {Library(False).list_book()[item][2]}")
            if option == 2:
                booksauthors = []
            ...
            if option == 7:
                for item in UserDB(False).list_user():
                    print(f"Name: {item} || Password: {UserDB(False).list_user()[item][0]} || Role: {UserDB(False).list_user()[item][1]}")
        ...
        while True:
            ...
            if option == 4:
                for item in Student(self.infolist[0],self.infolist[1]).show_borrowed_books():
                    print(f"Bid: {item[0]} ||| Name: {item[1][0]}\nNumber of Copies: {item[1][1]} ||| Authors: {item[1][2]}")
            if option == 5:
                break
            if input("Continue? (Y/N)") == "y":
                continue
